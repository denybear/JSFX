...
if not reaper.HasExtState(script_name, "joovs_playlist_index") then		-- no index in playlist exists already 
	...
	-- get track number in playlist, increment it, save it
	track_number_in_playlist = tonumber (reaper.GetExtState(script_name, "joovs_playlist_index"))
	if (track_number_in_playlist < #m) then
		track_number_in_playlist = track_number_in_playlist + 1			--	increment index to go to next song
	end
