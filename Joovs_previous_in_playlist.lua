...
if not reaper.HasExtState(script_name, "joovs_playlist_index") then		-- no index in playlist exists already 
	reaper.SetExtState(script_name, "joovs_playlist_index", "1", false)	-- default track number is 1 (first song from playlist)
	track_number_in_playlist = 1
else																	-- index in playlist exists already
	-- get track number in playlist, decrement it, save it
	track_number_in_playlist = tonumber (reaper.GetExtState(script_name, "joovs_playlist_index"))
	if (track_number_in_playlist > 1) then
		track_number_in_playlist = track_number_in_playlist - 1			--	decrement index to go to previous song
	end
	reaper.SetExtState(script_name, "joovs_playlist_index", tostring (track_number_in_playlist), false)
end
